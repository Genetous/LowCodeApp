...
export class ShortestPath extends Component {
    ...
    addNewNodeFieldSibling(index) {
        ...
        if (nodeCollectionInputs[index].nodeInputs.filter(ci => ci.field === "").length == 0) {
            nodeCollectionInputs[index].nodeInputs.push({ collection: "", key: "", value: "" })
            this.setState({
                nodeCollectionInputs
            })
        }
    ...
    addNewNodesSibling() {
        const { nodeCollectionInputs } = this.state;
        if (nodeCollectionInputs.length > 1)
            return;

        if (nodeCollectionInputs.filter(ci => ci.collection === "").length == 0) {
            this.setState({
                nodeCollectionInputs: [...this.state.nodeCollectionInputs, { collection: "", key: "", value: "" }]
            })
        }
            ...
            <CRow className='collection-siblings mb-1 mt-1' key={index}>
                <CCol md={12}>
                    <CForm>
                        <CRow>
                            <CCol md={5}>
                                <CFormInput type="text" name="key" id={sibling.key} value={sibling.key} placeholder="Key" onChange={this.nodesSiblingOnChangeHandler.bind(this, index, c_ind)} />
                            </CCol>
                            <CCol md={5}>
    ...
    nodesSiblingOnChangeHandler(index, e) {
        const { nodeCollectionInputs } = this.state;
        nodeCollectionInputs[index][e.target.name] = e.target.value;
        this.setState({
            nodeCollectionInputs
        });
        this.setCode(this.state.nodeCollectionInputs, "collectionString")
    ...
    getCollection() {
        return new Promise((resolve, reject) => {
            ...
            for (let i = 0; i < nodeCollectionInputs.length; i++) {
                var node = {}
                node["collection"] = nodeCollectionInputs[i].collection
                node["key"] = nodeCollectionInputs[i].key
                node["value"] = nodeCollectionInputs[i].value
                collection.values.push(node)
            }
            return resolve(collection.values);
        })
    }
    componentDidMount() {
        this.getCollections()
    ...
    async cmChanged(index, e) {
        const { nodeCollectionInputs } = this.state;
        nodeCollectionInputs[index]["collection"] = e.target.value;
        this.setState({
            nodeCollectionInputs
        });
        this.setCode(this.state.nodeCollectionInputs, "collectionString")
        ...
        return (
            <CRow>
                <CCard className="mb-4">
                    <CCardHeader style={{ color: '#4f5d73', backgroundColor: "#fff" }} component="h5" className='p-2'>Value - {index + 1}</CCardHeader>
                    <CCardBody className='m-1'>
                        <CRow>
                            <CCol sm={12} md={6}>
                                <CFormSelect onChange={this.cmChanged.bind(this, index)} value={this.state.nodeCollectionInputs[index].collectionName} name="field" as="select" className="mx-1 mb-1">
                        ...
                        <CRow className='collection-siblings mb-1 mt-1' key={index}>
                            <CCol md={12}>
                                <CForm>
                                    <CRow>
                                        <CCol md={6}>
                                            <CFormInput type="text" name="key" value={this.state.nodeCollectionInputs[index].key} placeholder="Key" onChange={this.nodesSiblingOnChangeHandler.bind(this, index)} className="mx-1 mb-1" />
                                        </CCol>
                                    </CRow>
                                    <CRow>
                                        <CCol md={6}>
                                            <CFormInput type="text" name="value" value={this.state.nodeCollectionInputs[index].value} placeholder="Value" onChange={this.nodesSiblingOnChangeHandler.bind(this, index)} className="mx-1 mb-3" />
                                        </CCol>
                                    </CRow>
                                </CForm>
                            </CCol>
    ...
    render() {
        ...
        return (
            <CContainer>
                <CCard className="mb-4">
                    <CCardHeader style={{ color: '#4f5d73', backgroundColor: "#fff" }} component="h5" className='p-2'>Find Shortest Path Between Two Nodes</CCardHeader>
                    <CCardBody className='m-1'>
                        {
                        ...
                        <CRow>
                            <CCol md={12}>
                                <CButton onClick={this.addNewNodesSibling.bind(this)} color="dark" className='mt-1 mb-3'><CIcon icon={icon.cilPlus} size="l" />Add New Value</CButton>
                            </CCol>
                        </CRow>
                        <CRow>
